...
do
  ...
  for file in "$ini" "$ETC/grids/$gridnick/Robust.ini" "$(realpath "$ini")"
  do
...
crudget $TMP.OpenSim.ini Const || crudget $TMP.OpenSim.ini const || end $? "could not get Const section from $EtcDirectory/OpenSim.ini"
