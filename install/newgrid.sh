...
gridnick=$(echo $gridname | tr "[:upper:]" "[:lower:]" | sed -r -e 's/(\W)/\L\1/g' -e 's/(^|[ _-])(\w)/\U\2/g')
...
then
  ...
  readvar answer && [ "${answer}" != "" ] || end $? "answer cannot be empty"
  if [ "$answer" = "y" ]
  then
