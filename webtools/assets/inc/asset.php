...
require_once('../assets/inc/cache.php');
...
function asset_get_zero($format) {
	 $image=IMAGE_ID_ZERO.".".strtolower($format);
	 if(!is_file($image)) {
	     $image=IMAGE_ID_ZERO . ".png";
	 }

	$h = fopen($image, "rb");
	$datas = fread($h, filesize($image));
	fclose($h);
...
function asset_get($asset_id, $format='JPEG') {
	...
	debug(ASSET_SERVER . $asset_id . " found");
