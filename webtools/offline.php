...
include("offline2018/config.php");
include("offline2018/mysql.php");
...
if ($method == "/SaveMessage/")
{
	...
	if ($start != -1)
	{
		...
		$DbLink->query("insert into ". OFFLINE_IM_TBL ." (uuid, message) values ('" .
		...
		if ($method == "/RetrieveMessages/")
		{
			...
			$DbLink->query("select message from ".OFFLINE_IM_TBL." where uuid='" .
			...
			echo "<?xml version=\"1.0\" encoding=\"utf-8\"?><ArrayOfGridInstantMessage xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\">";
				...
				$DbLink->query("delete from ".OFFLINE_IM_TBL." where uuid='" .
				mysql_escape_string($agent_id) . "'");
				exit;
			}
