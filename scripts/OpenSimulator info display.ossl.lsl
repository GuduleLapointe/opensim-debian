...
/*
// Old, tricky method
    integer Len = llStringLength((string)mem);
    if(Len == 8) // ##.### MB
    {
        string Mem1 = llSubStringIndex(mem,0,1);
        string Mem2 = llGetSubString(mem,2,4);
        mem = Mem1 + "." + Mem2 + " Mb";
    } else if(Len == 9) //###.### MB
    {
        string Mem1 = llGetSubString(mem,0,2);
        string Mem2 = llGetSubString(mem,3,5);
        mem = Mem1 + "." + Mem2 + " Mb";
    } else if(Len == 10) //#.### GB
    {
    string Mem1 = llGetSubString(mem,0,0);
    string Mem2 = llGetSubString(mem,1,3);
    mem = Mem1 + "." + Mem2 + " Gb";
    } else if(Len == 11) //##.### GB
    {
        string Mem1 = llGetSubString(mem,0,1);
        string Mem2 = llGetSubString(mem,2,4);
        mem = Mem1 + "." + Mem2 + " Gb";
    }
    formatted += "\nold style:" + mem;
...
GenStats()
{
    ...
    text += "\nVersion: " + strReplaceUntilNoMatch(osGetSimulatorVersion(), "  ", " ");
    display(text);
}
